...
describe TrackingMailer, type: :mailer do
  ...
  describe "Welcome email" do
    ...
    context 'when the tracking has an end date' do
      it do
        tracking = create(:tracking, date_behavior: :earlier, end_date: Date.today)

        TrackingMailer.welcome(tracking).deliver_now

        email = ActionMailer::Base.deliveries.first
        expect(email.body.raw_source).to include(I18n.l tracking.end_date)
      end
    end

    context 'when the tracking has an begin date' do
      it do
        tracking = create(:tracking, date_behavior: :later, begin_date: Date.today)

        TrackingMailer.welcome(tracking).deliver_now

        email = ActionMailer::Base.deliveries.first
        expect(email.body.raw_source).to include(I18n.l tracking.begin_date)
      end
    end
  end

  context 'Alert mail' do
    ...
    context 'when the tracking has an end date' do
      it do
        tracking = create(:tracking_with_availabilities, date_behavior: :earlier, end_date: Date.today + 1.day)
    ...
    context 'when the tracking has an begin date' do
      it do
        tracking = create(:tracking_with_availabilities, date_behavior: :later, begin_date: Date.today)
